    ...
    def test_http_restore_invalidates_old_page(self):
        ...
        with patch.multiple(app, THESIS_DIR=self.root, MARKDOWN_ROOT=Path(self.temp.name),
            ...
            try:
                ...
                cache = app.preview_directory(); cache.mkdir(parents=True)
                (cache/'latest.pdf').write_bytes(b'%PDF-old')
                (cache/'latest-map.json').write_text('{}')
                ...
                self.assertFalse((cache/'latest.pdf').exists())
                self.assertFalse((cache/'latest-map.json').exists())
                self.assertEqual((self.root/'one.md').read_text(encoding='utf-8'),'旧章节')
